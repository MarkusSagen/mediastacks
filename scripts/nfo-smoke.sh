...
# movie.nfo is written in the movie folder, then `medias undo` removes it.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"; cd "$ROOT"
MEDIAS="$ROOT/zig-out/bin/medias"
[[ -x "$MEDIAS" ]] || { echo "build first: zig build" >&2; exit 2; }
TMP="$(mktemp -d -t mediastacks-nfo.XXXXXX)"; trap 'rm -rf "$TMP"' EXIT
...
"$MEDIAS" organize "$SRC" --to "$LIB" --nfo --offline >/dev/null
