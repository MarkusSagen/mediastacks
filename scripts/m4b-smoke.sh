#!/usr/bin/env bash
# makem4b smoke: ffmpeg makes 3 tagged chapter mp3s + a cover; `shelve makem4b`
# merges them into one chaptered .m4b; ffprobe confirms 3 chapters + audio +
# attached cover; sources are kept; `shelve undo` removes the .m4b.
set -euo pipefail
...
TMP="$(mktemp -d -t stacks-m4b.XXXXXX)"; trap 'rm -rf "$TMP"' EXIT
...
OUT="$("$SHELVE" makem4b "$SRC" --to "$LIB")"; echo "$OUT" | sed 's/^/  > /'
