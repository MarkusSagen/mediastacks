...
fn handle(io: std.Io, session: *Session, env: *std.process.Environ.Map, request: *std.http.Server.Request) !void {
    ...
    if (std.mem.eql(u8, path, "/api/apply")) {
        ...
        const res = try apply_mod.apply(session.arena, session.plan, .skip, env, .{ .write = session.cfg.write_tags }, session.cfg.emit_ignore, session.cfg.write_nfo);
...
fn mkSession(a: std.mem.Allocator) Session {
    ...
    return .{ .arena = a, .cfg = .{ .library_root = "/lib", .tv_template = config.DEFAULT_TV, .movie_template = config.DEFAULT_MOVIE, .music_template = config.DEFAULT_MUSIC, .audiobook_template = config.DEFAULT_AUDIOBOOK, .comic_template = config.DEFAULT_COMIC }, .plan = .{ .library_root = "/lib", .source = "/x", .groups = groups } };
}
