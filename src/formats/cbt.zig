//! CBT handler — TAR-of-images comic archive. Rare in modern
//! collections (TAR offers no compression on its own, so producers
//! prefer CBZ or CBR) but the format exists in older archives.
//!
//! Same shell-out shape as CBR — see `cbr.zig`. `7zz` handles plain
//! TAR + the compressed variants (.tar.gz, .tar.bz2) transparently.
